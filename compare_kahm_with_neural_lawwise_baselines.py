#!/usr/bin/env python3
"""
compare_kahm_with_neural_lawwise_baselines.py

Purpose
-------
Extend the existing KAHM-vs-baselines comparison with compact neural baselines
while preserving the same evaluation contract and the same distributed-law
training / distance-gated serving structure:

    cheap lexical query features (IDF–SVD) -> predicted Mixedbread query embedding

The script keeps the law-wise split and gating mechanism of
compare_kahm_with_stronger_baselines.py and adds two neural baselines:

1) mlp_regressor_lawwise:
   A compact law-wise multi-output MLP regressor mapping IDF–SVD query features
   to the Mixedbread query-embedding space. Serving keeps the same law gating as
   the existing baselines: choose the law whose predicted embedding is closest to
   that law's teacher-space prototype set.

2) mlp_proto_lawwise:
   A compact law-wise MLP classifier predicting posterior mass over semantic
   prototypes learned from teacher embeddings. The final embedding is the
   posterior-weighted prototype mixture. This is the neural counterpart of the
   logistic prototype-mixture baseline.

The script can optionally load KAHM models or precomputed KAHM embeddings and
produces the same style of retrieval report and JSON summary as the existing
comparison script.

Why this design?
----------------
This broadens the reviewer-facing baseline set *without changing the deployment
contract*. It therefore answers the question:

    "Does KAHM beat non-neural and compact neural lexical->semantic adapters
     under the same distributed law-wise serving setup?"

It does *not* attempt to replace the separate direct-transformer reference,
which is still represented by Mixedbread(true query embeddings).
"""
...
def safe_validation_fraction(n_samples: int, requested: float) -> Optional[float]:
    ...
    # Need at least ~2 validation samples and still enough training points.
...
def expand_proba_to_all_classes(proba: np.ndarray, seen_classes: np.ndarray, n_classes: int) -> np.ndarray:
    out = np.zeros((proba.shape[0], int(n_classes)), dtype=np.float32)
    cols = np.asarray(seen_classes, dtype=np.int64)
    out[:, cols] = np.asarray(proba, dtype=np.float32)
    return out
...
def train_mlp_regressor_lawwise(
    x_train: np.ndarray,
    y_train: np.ndarray,
    train_laws: Sequence[str],
    *,
...
) -> Dict[str, MLPRegressorLawModel]:
    laws = sorted(set(str(lw) for lw in train_laws))
    models: Dict[str, MLPRegressorLawModel] = {}
    train_laws_arr = np.asarray([str(lw) for lw in train_laws], dtype=object)
    ...
    for law in laws:
        ...
        vf = safe_validation_fraction(x_l.shape[0], validation_fraction) if early_stopping else None
        reg = MLPRegressor(
            ...
            early_stopping=bool(vf is not None),
            validation_fraction=float(vf) if vf is not None else 0.1,
            n_iter_no_change=12,
            random_state=int(seed),
        )
        reg.fit(x_l, y_l)
        prototypes, _labels = make_semantic_prototypes(y_l, n_prototypes=n_prototypes, seed=seed)
        models[law] = MLPRegressorLawModel(law=law, reg=reg, prototypes=prototypes)
    return models



def predict_mlp_regressor_lawwise(models: Mapping[str, MLPRegressorLawModel], x_test: np.ndarray) -> PredictionBundle:
    names = sorted(models.keys())
    if not names:
        raise ValueError("No MLP-regressor models available")

    pred_by_name: Dict[str, np.ndarray] = {}
    score_by_name: Dict[str, np.ndarray] = {}
    for name in names:
        model = models[name]
        y_hat = model.reg.predict(x_test)
...
def train_mlp_proto_lawwise(
    ...
    *,
    ...
    validation_fraction: float,
    learning_rate_init: float,
) -> Dict[str, MLPPrototypeLawModel]:
    laws = sorted(set(str(lw) for lw in train_laws))
    models: Dict[str, MLPPrototypeLawModel] = {}
    train_laws_arr = np.asarray([str(lw) for lw in train_laws], dtype=object)
    ...
    for law in laws:
        ...
        if uniq.size <= 1:
            ...
            clf.fit(x_l, labels)
        models[law] = MLPPrototypeLawModel(law=law, clf=clf, prototypes=prototypes, cluster_labels=labels)
    return models



def predict_mlp_proto_lawwise(models: Mapping[str, MLPPrototypeLawModel], x_test: np.ndarray) -> PredictionBundle:
    names = sorted(models.keys())
    if not names:
        raise ValueError("No MLP-prototype models available")
    ...
    for name in names:
        ...
        if model.clf is None:
            ...
            raw_proba = model.clf.predict_proba(x_test)
            raw_proba = np.asarray(raw_proba, dtype=np.float32)
            if raw_proba.ndim != 2:
                raise ValueError(f"Unexpected predict_proba shape for {name}: {raw_proba.shape}")
            proba = expand_proba_to_all_classes(raw_proba, np.asarray(model.clf.classes_), model.prototypes.shape[0])
            y_hat = (proba @ model.prototypes).astype(np.float32, copy=False)
        y_hat = l2_normalize_rows(y_hat).astype(np.float32, copy=False)
        pred_by_name[name] = y_hat
        score_by_name[name] = pairwise_min_sqdist(y_hat, model.prototypes)
...
def build_markdown_report(
    *,
    ...
    lines.append("")
    lines.append("The added neural baselines remain law-wise models with the same prototype-distance gating used for the existing non-neural baselines, so the distributed-law serving setup is unchanged.")
    lines.append("")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Compare KAHM against stronger baselines, including compact neural law-wise adapters.")
    ...
    p.add_argument(
        "--baseline_methods",
        default="mlp_regressor,mlp_proto",
        help="Comma-separated subset of: ridge,logistic_proto,mlp_regressor,mlp_proto",
    )
    ...
    p.add_argument("--mlp_validation_fraction", type=float, default=0.1)

    p.add_argument("--out_dir", default="neural_baseline_comparison")
    p.add_argument("--save_baseline_query_embeddings", action="store_true")
    p.add_argument("--report_path", default="neural_baseline_comparison/report.md")
    p.add_argument("--results_json_path", default="neural_baseline_comparison/results.json")
    return p
...
def main() -> int:
    ...
    allowed = {"ridge", "logistic_proto", "mlp_regressor", "mlp_proto"}
    ...
    if "mlp_regressor" in baseline_methods:
        print("Training mlp_regressor_lawwise baseline ...")
        models_mlp_reg = train_mlp_regressor_lawwise(
            x_train,
            y_train,
            train_laws,
            n_prototypes=int(args.baseline_n_prototypes),
            seed=int(args.seed),
            hidden_sizes=parse_hidden_sizes(str(args.mlp_reg_hidden)),
            alpha=float(args.mlp_reg_alpha),
            max_iter=int(args.mlp_reg_max_iter),
            early_stopping=bool(args.mlp_early_stopping),
            validation_fraction=float(args.mlp_validation_fraction),
            learning_rate_init=float(args.mlp_reg_learning_rate_init),
        )
        mlp_reg_pred = predict_mlp_regressor_lawwise(models_mlp_reg, x_test)
        embedding_predictions["mlp_regressor_lawwise(query→MB corpus)"] = mlp_reg_pred.embeddings
        embedding_metrics["mlp_regressor_lawwise(query→MB corpus)"] = compute_embedding_metrics(mlp_reg_pred.embeddings.T, y_test.T)
        if args.save_baseline_query_embeddings:
            np.savez_compressed(
                str(out_dir / "mlp_regressor_query_embeddings_test.npz"),
                query_id=np.asarray(test_ids, dtype=np.str_),
                embeddings=np.asarray(mlp_reg_pred.embeddings, dtype=np.float32),
            )

    if "mlp_proto" in baseline_methods:
        print("Training mlp_proto_lawwise baseline ...")
        models_mlp_proto = train_mlp_proto_lawwise(
            x_train,
            y_train,
            train_laws,
            n_prototypes=int(args.baseline_n_prototypes),
            seed=int(args.seed),
            hidden_sizes=parse_hidden_sizes(str(args.mlp_proto_hidden)),
            alpha=float(args.mlp_proto_alpha),
            max_iter=int(args.mlp_proto_max_iter),
            early_stopping=bool(args.mlp_early_stopping),
            validation_fraction=float(args.mlp_validation_fraction),
            learning_rate_init=float(args.mlp_proto_learning_rate_init),
        )
        mlp_proto_pred = predict_mlp_proto_lawwise(models_mlp_proto, x_test)
        embedding_predictions["mlp_proto_lawwise(query→MB corpus)"] = mlp_proto_pred.embeddings
        embedding_metrics["mlp_proto_lawwise(query→MB corpus)"] = compute_embedding_metrics(mlp_proto_pred.embeddings.T, y_test.T)
        if args.save_baseline_query_embeddings:
            np.savez_compressed(
                str(out_dir / "mlp_proto_query_embeddings_test.npz"),
                query_id=np.asarray(test_ids, dtype=np.str_),
                embeddings=np.asarray(mlp_proto_pred.embeddings, dtype=np.float32),
            )
