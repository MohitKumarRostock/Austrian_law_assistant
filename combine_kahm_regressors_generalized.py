...
def combine_kahm_regressors_distance_gated_multi(
    ...
    *,
    ...
    for i, model in enumerate(model_list):
        Y_i_col, d_i = kahm_predict_with_min_distance(
            model,
            X_col,
            mode=mode,
            alpha=alpha,
        ...
        if Y_best_col is None:
            Y_best_col = np.asarray(Y_i_col, dtype=np.float32)
            best_score[:] = d_i
            chosen[:] = np.int16(i)
        else:
            if Y_i_col.shape != Y_best_col.shape:
                raise ValueError(
                    f"Output shape mismatch: model {names[i]} produced {Y_i_col.shape}, "
                    f"but previous models produced {Y_best_col.shape}."
                )
