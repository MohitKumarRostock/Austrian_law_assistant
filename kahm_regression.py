...
def train_kahm_regressor(
    ...
    # ----------------
    # When KMeans yields singleton output clusters, OTFL autoencoder training can fail.
    # The legacy approach merged singletons into the nearest non-singleton cluster (reducing C_eff).
    # For regression, reducing C_eff can change the model capacity and degrade performance.
    #
    # Strategy:
    #   - "augment" (default): keep the cluster count stable by *adding* one auxiliary sample
    #     to each singleton cluster: X_aux is chosen to be similar to the singleton's X,
    #     while Y_aux is an identical copy of the singleton's Y.
    #   - "merge": legacy behavior; merges singleton samples into a nearby non-singleton cluster.
    singleton_strategy: str = "augment",  # {'augment','merge'}
    # Mix factor for constructing X_aux = (1-mix)*X_singleton + mix*X_neighbor (0..1).
    # Smaller values keep X_aux close to the singleton input while still providing a second point.
    ...
    if singletons.size > 0 and verbose:
        print(f"Handling {singletons.size} singleton cluster(s) using strategy='{singleton_strategy}'...")
    ...
    if singletons.size > 0:
        ...
        if strategy == "merge":
            ...
            # Preferred behavior: keep cluster count stable by *augmenting* each singleton cluster.
            #
            # For each singleton cluster c with (x_s, y_s), we add one auxiliary sample (x_aux, y_s):
            #   - y_s is copied verbatim (identical output)
            #   - x_aux is constructed to be close to x_s, using a neighbor x_n from nearby output clusters.
            #
            # This avoids shrinking the number of effective clusters while satisfying OTFL's
            # minimum-2-samples-per-cluster requirement.
            rng = np.random.RandomState(int(random_state) if random_state is not None else 0)

            # Build cluster members once (fast lookup).
            ...
            # Precompute global nearest neighbors in *input* space using Euclidean distance.
            # This implements the intended behavior for singleton augmentation:
            # for each singleton sample i, choose its nearest other training sample j in X-space.
            #
            # Note: we query k>2 to robustly handle duplicate points; we then select the first neighbor != itself.
            X_nn = np.asarray(X.T, dtype=np.float32 if X.dtype == np.float64 else X.dtype)
            N_nn = int(X_nn.shape[0])
            if N_nn < 2:
                raise RuntimeError("Cannot augment singleton clusters with fewer than 2 total training points.")
            ...
            if np.any(self_mask):
                for kk in range(2, nn_idx.shape[1]):
                    ...
                    if not np.any(self_mask):
                        break
            if np.any(self_mask):
                # Extremely degenerate case (e.g., N=2 with identical indexing anomalies): fall back to a different index.
                bad = np.where(self_mask)[0]
                for i in bad:
                    nearest_global[int(i)] = int((int(i) + 1) % N_nn)

            # We will append samples; collect them first to avoid repeated reallocations.
            ...
            for cl in singletons:
                ...
                if nn_local == s_idx:
                    # Fallback (should be rare after precomputation safeguards).
                    nn_local = int((s_idx + 1) % int(X.shape[1]))
                x_neighbor = X[:, nn_local]

                # Construct auxiliary input close to singleton input.
                x_aux = (1.0 - mix) * x_singleton + mix * x_neighbor

                # Preserve singleton input norm (helps when inputs are approximately unit-norm embeddings).
                x_norm = float(np.linalg.norm(x_singleton)) + 1e-12
                aux_norm = float(np.linalg.norm(x_aux)) + 1e-12
                x_aux = x_aux * (x_norm / aux_norm)

                # Append new column: identical output, similar input
                X_new_cols.append(x_aux.reshape(-1, 1))
                Y_new_cols.append(Y[:, s_idx].reshape(-1, 1))  # identical output (D_out,1)
            ...
            if X_new_cols:
                ...
                labels_zero_based = np.concatenate(
                    [labels_zero_based, np.asarray(new_labels, dtype=labels_zero_based.dtype)], axis=0
                )
                # Update counts after augmentation
                counts = np.bincount(labels_zero_based, minlength=n_clusters)

            # Safety check: OTFL requires >=2 samples per cluster post-processing.
            remaining_singletons = np.where(counts == 1)[0]
            if remaining_singletons.size > 0:
                raise RuntimeError(
                    f"Augmentation failed to eliminate {remaining_singletons.size} singleton cluster(s). "
                    "Consider reducing n_clusters or check that X contains at least two distinct points."
                )
...
def tune_cluster_centers_nlms(
    ...
    *,
    ...
    epsilon:
        ...
        If None, (good general default: 1).
    epochs:
