...
def build_idf_svd_pipeline(
    *,
    ...
    if tfidf_dtype is not None:
        word_params["dtype"] = tfidf_dtype
    word_vectorizer = TfidfVectorizer(**word_params)

    transformer_list = [("word_tfidf", word_vectorizer)]
    ...
    if use_char_ngrams:
        ...
        if tfidf_dtype is not None:
            char_params["dtype"] = tfidf_dtype
        char_vectorizer = TfidfVectorizer(**char_params)
        transformer_list.append(("char_tfidf", char_vectorizer))
...
def build_embedding_index_idf_svd_npz_and_model(
    *,
    ...
    meta = {
        ...
        "tfidf_dtype": str(tfidf_np_dtype) if tfidf_np_dtype is not None else "auto",
...
def parse_args(argv) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default="float32",
        choices=["auto", "float16", "float32", "float64"],
        help="Embeddings dtype written to NPZ. Default 'auto' preserves pipeline output dtype.",
    )
    p.add_argument(
        "--union_n_jobs",
        type=int,
...
def main(argv=None) -> int:
    ...
    cfg = IDFSVDConfig(
        ...
        min_df_word=(args.min_df_word if args.min_df_word is not None else args.min_df),
        max_df_word=(args.max_df_word if args.max_df_word is not None else args.max_df),
        min_df_char=(args.min_df_char if args.min_df_char is not None else args.min_df),
        max_df_char=(args.max_df_char if args.max_df_char is not None else args.max_df),
