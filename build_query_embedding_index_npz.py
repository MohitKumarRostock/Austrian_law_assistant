...
Precompute Mixedbread embeddings for TRAIN_QUERY_SET and TEST_QUERY_SET (from query_set.py)
and save them to *two* NPZ files.

Why two NPZ files?
-----------------
Keeping the downstream regression training torch-free is easiest if TRAIN and TEST targets
are fully precomputed and stored separately.

Output NPZ (per split) contains:
...
---------------------
If you pass --out queries_embedding_index.npz, the script writes:
...
DEFAULT_OUT_TRAIN = "queries_embedding_index_train.npz"
DEFAULT_OUT_TEST = "queries_embedding_index_test.npz"
...
def _load_query_sets() -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Load TRAIN/TEST query sets from query_set.py."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default=DEFAULT_OUT,
        help=(
            "Output NPZ prefix or file name. If --out_train/--out_test are not provided, "
            "the script writes <out>_train.npz and <out>_test.npz. "
    ...
    p.add_argument(
        "--out_train",
        default="",
        help=f"Explicit output NPZ for TRAIN_QUERY_SET (default: derived; e.g., {DEFAULT_OUT_TRAIN})",
    )
    p.add_argument(
        "--out_test",
        default="",
        help=f"Explicit output NPZ for TEST_QUERY_SET (default: derived; e.g., {DEFAULT_OUT_TEST})",
    )
    ...
    p.add_argument(
        "--train_only",
        action="store_true",
        help="Only write the TRAIN split NPZ (skip TEST).",
    )
...
def _derive_split_paths(out: Path, out_train: str, out_test: str) -> Tuple[Path, Path]:
    """Derive train/test output paths.

    If out_train/out_test are set, use them.
    Else derive from `out` by appending _train/_test before .npz.
    """
    if out_train.strip():
        train_path = _ensure_npz_suffix(Path(out_train.strip()))
    else:
        out_npz = _ensure_npz_suffix(out)
        stem = out_npz.with_suffix("")
        train_path = _ensure_npz_suffix(Path(str(stem) + "_train"))

    if out_test.strip():
        test_path = _ensure_npz_suffix(Path(out_test.strip()))
    else:
        out_npz = _ensure_npz_suffix(out)
        stem = out_npz.with_suffix("")
        test_path = _ensure_npz_suffix(Path(str(stem) + "_test"))
...
def main() -> int:
    ...
    train_out, test_out = _derive_split_paths(Path(args.out), str(args.out_train), str(args.out_test))

    # Overwrite behavior: if any requested output exists and overwrite is not set, exit.
    to_check = [train_out] + ([] if bool(args.train_only) else [test_out])
    existing = [p for p in to_check if p.exists()]
    if existing and not args.overwrite:
        print("Output exists (use --overwrite to replace):")
        for p in existing:
            print(f"  - {p}")
        return 0

    train_qs, test_qs = _load_query_sets()
    train_ids, train_texts = _validate_query_fields(train_qs)
    test_ids, test_texts = _validate_query_fields(test_qs)

    # Encode in a single call for efficiency (same model, same prefix, consistent normalization).
    all_ids = train_ids + ([] if bool(args.train_only) else test_ids)
    all_texts = train_texts + ([] if bool(args.train_only) else test_texts)
    ...
    print(
        f"Embedding {len(all_texts)} queries (train={len(train_texts)}, test={0 if bool(args.train_only) else len(test_texts)}) "
        f"with model: {model_name} (device={device}, dim={dim}, batch={batch})"
    )
    ...
    if args.stream:
        Y_all = _encode_streaming(
            model=model,
            query_texts=all_texts,
            prefix=prefix,
            batch_size=batch,
            dim=dim,
    ...
    if args.normalize:
        l2_normalize_rows_inplace(Y_all)

    # Split
    n_train = len(train_texts)
    Y_train = Y_all[:n_train]
    Y_test = None if bool(args.train_only) else Y_all[n_train:]
    ...
    base_meta = dict(
        ...
        note="L2-normalized query embeddings for query_set.py splits; generated separately to keep regression torch-free.",
    )

    print(f"Saving TRAIN: {train_out} (compress={bool(args.compress)})")
    _save_npz_atomic(
        train_out,
        compress=bool(args.compress),
        split=np.array(["train"], dtype=np.str_),
        query_id=np.array(train_ids, dtype=np.str_),
        query_text=np.array(train_texts, dtype=np.str_),
        embeddings=Y_train,
        n_queries=int(len(train_texts)),
        **base_meta,
    )

    if not bool(args.train_only):
        assert Y_test is not None
        print(f"Saving TEST:  {test_out} (compress={bool(args.compress)})")
        _save_npz_atomic(
            test_out,
            compress=bool(args.compress),
            split=np.array(["test"], dtype=np.str_),
            query_id=np.array(test_ids, dtype=np.str_),
            query_text=np.array(test_texts, dtype=np.str_),
            embeddings=Y_test,
            n_queries=int(len(test_texts)),
            **base_meta,
        )
